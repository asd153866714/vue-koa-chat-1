<template>
  ...
  <div class="wrapper">
    ...
    <the-header goback="true" chatTitle="使用者資料"></the-header>
    <div class="main"></div>
    <div class="content">
      ...
      <p class="href">
        ...
        <span @click="goWebsite"
          ><svg class="icon" aria-hidden="true">
            <use xlink:href="#icon-web"></use></svg
        ></span>
      </p>
      <p v-if="this.isMyFriend">
        <svg class="icon" aria-hidden="true">
          <use xlink:href="#icon-tianjiabeizhu"></use></svg
      ...
      <p>
        <svg class="icon" aria-hidden="true">
          <use xlink:href="#icon-placeholder"></use></svg
    ...
    >
      ...
      <span class="go-chat" @click="goChat">傳送訊息</span>
    </div>
    <div v-if="this.isMe" class="action">
      <span class="editor-info" @click="editorInfo">编辑使用者資料</span>
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    //同意加好友
    async agreeBeFriend() {
      await axios.post("/api/be_friend", {
        // user_id: this.myInfo.user_id,
        other_user_id: this.$route.params.user_id,
      });
    ...
    async updateNewFriends() {
      await axios.put("/api/update_newfriends", {
    ...
    confirm(value) {
      ...
      //         value.myInfo.website.substr(0, 4) != "http"
      //           ? "http://" + value.myInfo.website
      //           : value.myInfo.website;
      //     }
      //   }
      ...
      //     } else {
      //       value.myInfo.github =
      //         value.myInfo.github.substr(0, 4) != "http"
      //           ? "http://" + value.myInfo.github
      //           : value.myInfo.github;
      //     }
      //   }
      //   console.log(value.myInfo);
      //   axios
      //     .put("/api/v1/editor_info", {
  ...
  async created() {
    ...
    this.isMe = this.myInfo.id == this.$route.params.user_id ? true : false;
